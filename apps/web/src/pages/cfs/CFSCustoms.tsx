...
const HOLD_BADGE: Record<string, { label: string; color: string; icon: string }> = {
  ...
  EPA: { label: 'EPA', color: 'bg-teal-700 text-white', icon: '♻️' },
      ...
      {activeTab === 'exams' && (
        <div>
          {MOCK_ENTRIES.filter(e => e.examType).length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-lg p-8 text-center"><p className="text-2xl mb-2">✅</p><p className="text-sm text-gray-500">No CBP exams currently scheduled</p></div>
          ) : (
            <div className="space-y-3">
        ...
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setSelectedEntry(null)}>
          ...
          <div className="relative w-[500px] bg-white shadow-2xl h-full overflow-y-auto border-l border-gray-200" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between mb-2"><h3 className="text-sm font-bold text-gray-900">{selectedEntry.entryNumber || selectedEntry.orderNumber}</h3><button onClick={() => setSelectedEntry(null)} className="text-gray-400 hover:text-gray-600 text-lg">×</button></div>
      ...
      {showACE && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowACE(false)}>
          <div className="bg-white rounded-xl w-full max-w-3xl max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-green-500" /><div><p className="text-sm font-bold text-green-800">ACE Connected — Gemini Express Transport Corp</p><p className="text-xs text-green-600">Filer Code: AGS · Bond Type: Continuous · Last Sync: 2 min ago</p></div></div>
              ...
              <div className="bg-gray-50 rounded-lg p-3"><h4 className="text-xs font-semibold text-gray-700 mb-2">ACE Capabilities</h4><div className="grid grid-cols-2 gap-2 text-xs text-gray-600"><span>✓ Entry Summary (Type 01/11)</span><span>✓ ISF 10+2 Filing</span><span>✓ CBP Cargo Release</span><span>✓ FDA Prior Notice</span><span>✓ PGA Agency Messages</span><span>✓ Duty / Tax Calculation</span><span>✓ Reconciliation</span><span>✓ Protest Filing</span></div></div>
            </div>
