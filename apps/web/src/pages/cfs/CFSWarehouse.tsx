...
const MOCK_INVENTORY: WarehouseItem[] = [
  { id: 'wi1', hawb: 'GE-H-20260414-002A', mawb: '180-99321100', orderNumber: 'JFK-IMP-0902', shipper: 'Global Textile Co.', consignee: 'NY Fashion Import Inc.', consigneeCity: 'New York', consigneeState: 'NY', commodity: 'Textile / Garments', pieces: 12, weight: '1,400 kg', dims: '80x60x80 cm', zone: 'A', location: 'A-14-02', receivedAt: '2026-04-14T10:30:00Z', storageDays: 0, status: 'DECONSOLIDATING', actionNeeded: 'Decon in progress â€” 6 sub-lots', temperature: 'Ambient', hazmat: false, highValue: false },
  ...
  { id: 'wi9', hawb: 'GE-E-20260414-002', mawb: '176-88100502', orderNumber: 'JFK-EXP-0502', shipper: 'NY Fashion Export LLC', consignee: 'HK Garment Import Co.', consigneeCity: 'Hong Kong', consigneeState: 'HK', commodity: 'Textile / Fashion (Export)', pieces: 32, weight: '4,200 kg', dims: '80x60x80 cm', zone: 'A', location: 'A-EXPORT-05', receivedAt: '2026-04-14T07:00:00Z', storageDays: 0, status: 'CONSOLIDATING', actionNeeded: 'Export consol â€” 4 shippers â†’ 1 MAWB', temperature: 'Ambient', hazmat: false, highValue: false },
];
...
const MOCK_RECEIVING: ReceivingRecord[] = [
  ...
  { id: 'rc2', orderNumber: 'JFK-IMP-0902', mawb: '180-99321100', airline: 'Cathay Pacific', flightNumber: 'CX 840', airport: 'JFK', terminal: 'Bldg 151', expectedPieces: 48, receivedPieces: 48, receivedAt: '2026-04-14T10:30:00Z', receivedBy: 'Tony Russo', condition: 'GOOD', notes: 'Received â€” deconsolidation started', status: 'COMPLETE' },
  { id: 'rc3', orderNumber: 'ORD-IMP-0450', mawb: '618-77200100', airline: 'ANA', flightNumber: 'NH 112', airport: 'ORD', terminal: 'Cargo F', expectedPieces: 8, receivedPieces: 8, receivedAt: '2026-04-14T09:00:00Z', receivedBy: 'Carlos Mendez', condition: 'GOOD', notes: 'DG â€” MSDS verified at receiving. Stored in Zone C.', status: 'COMPLETE' },
  { id: 'rc4', orderNumber: 'MIA-IMP-0325', mawb: '235-22100400', airline: 'LATAM', flightNumber: 'LA 502', airport: 'MIA', terminal: 'MIA Cargo', expectedPieces: 20, receivedPieces: 18, receivedAt: '2026-04-14T11:00:00Z', receivedBy: 'Maria Santos', condition: 'PARTIAL', notes: '2 pieces short â€” carrier signed exception. Claim being filed.', status: 'DISCREPANCY' },
  { id: 'rc5', orderNumber: 'JFK-IMP-0908', mawb: '074-11200500', airline: 'KLM', flightNumber: 'KL 642', airport: 'JFK', terminal: 'Bldg 22', expectedPieces: 15, receivedPieces: 0, receivedAt: '', receivedBy: '', condition: 'GOOD', notes: 'Flight arrived â€” pickup dispatched. ETA 3:30 PM.', status: 'EXPECTED' },
  { id: 'rc6', orderNumber: 'DFW-IMP-0212', mawb: '297-55100600', airline: 'Lufthansa', flightNumber: 'LH 438', airport: 'DFW', terminal: 'DFW Cargo', expectedPieces: 10, receivedPieces: 5, receivedAt: '2026-04-14T13:30:00Z', receivedBy: 'James Harper', condition: 'DAMAGED', notes: 'In progress â€” 2 cartons show water damage. Photos taken.', status: 'IN_PROGRESS' },
];
...
const MOCK_CONSOL: ConsolJob[] = [
  ...
  { id: 'cj3', type: 'DECONSOLIDATION', jobNumber: 'DCN-0041', mawb: '131-55100700', hawbs: ['GE-H-20260413-020A', 'GE-H-20260413-020B', 'GE-H-20260413-020C'], status: 'COMPLETE', totalPieces: 30, processedPieces: 30, startedAt: '2026-04-13T14:00:00Z', completedAt: '2026-04-13T16:30:00Z', assignedTo: 'Tony Russo', notes: 'Complete â€” all 3 lots staged for delivery' },
  { id: 'cj4', type: 'CONSOLIDATION', jobNumber: 'CON-0087', mawb: '180-66100800', hawbs: ['EXP-HAWB-010', 'EXP-HAWB-011'], status: 'COMPLETE', totalPieces: 18, processedPieces: 18, startedAt: '2026-04-13T08:00:00Z', completedAt: '2026-04-13T10:00:00Z', assignedTo: 'Maria Santos', notes: 'Consolidated for KE 081 to ICN â€” departed' },
];
...
function fmtDT(d: string) { return d ? new Date(d).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'â€”'; }
...
export function CFSWarehouse() {
  ...
  return (
    <div>
      ...
      {/* â”€â”€ Inventory Tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {activeTab === 'inventory' && (
        <div>
          <div className="flex gap-1 mb-3">
            {['All', 'A', 'B', 'C', 'D', 'E'].map(z => (
              <button key={z} onClick={() => setZoneFilter(z)} className={`px-3 py-1 text-xs rounded font-medium ${zoneFilter === z ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{z === 'All' ? 'All Zones' : `Zone ${z} â€” ${ZONE_NAMES[z]}`}</button>
            ))}
          </div>
            ...
            <table className="w-full text-xs"><thead><tr className="bg-gray-50 border-b border-gray-200">
              ...
              {filteredInventory.map(i => (
                <tr key={i.id} className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${i.hazmat ? 'bg-red-50' : i.highValue ? 'bg-yellow-50' : i.storageDays >= 2 ? 'bg-orange-50' : ''}`} onClick={() => setSelectedItem(i)}>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">{i.commodity}{i.hazmat && <span className="ml-1 text-red-600 font-bold">â˜£</span>}{i.highValue && <span className="ml-1 text-yellow-600 font-bold">ðŸ’Ž</span>}</td>
      ...
      {/* â”€â”€ Receiving Tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {activeTab === 'receiving' && (
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <table className="w-full text-xs"><thead><tr className="bg-gray-50 border-b border-gray-200">
            ...
            {MOCK_RECEIVING.map(r => (
              <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 ${r.status === 'DISCREPANCY' ? 'bg-red-50 border-l-4 border-l-red-400' : r.status === 'EXPECTED' ? 'bg-blue-50' : ''}`}>
                ...
                <td className="px-3 py-2.5 text-right font-medium"><span className={r.receivedPieces < r.expectedPieces && r.receivedPieces > 0 ? 'text-red-600' : 'text-gray-900'}>{r.receivedPieces || 'â€”'}</span></td>
                <td className="px-3 py-2.5">{r.receivedPieces > 0 ? <span className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${COND_BADGE[r.condition]}`}>{r.condition}</span> : 'â€”'}</td>
                <td className="px-3 py-2.5 text-gray-600">{r.receivedBy || 'â€”'}</td>
      ...
      {/* â”€â”€ Consolidation / Deconsolidation Tab â”€â”€â”€â”€â”€â”€â”€ */}
      ...
      {/* â”€â”€ Storage Zones Tab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {activeTab === 'storage' && (
              ...
              <div key={zone} className="bg-white border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-sm font-bold text-gray-900">Zone {zone} â€” {ZONE_NAMES[zone]}</p><p className="text-xs text-gray-400">{stats.items} items Â· {stats.pieces} pieces</p></div>
                  </div>
                    ...
                    {items.map(i => (
                      <div key={i.id} className="flex items-center justify-between py-1.5 px-3 bg-gray-50 rounded text-xs">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-800">{i.commodity}{i.hazmat && ' â˜£'}{i.highValue && ' ðŸ’Ž'}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-gray-600">{i.pieces} pcs Â· {i.weight}</span>
      ...
      {/* â”€â”€ Inventory Detail Flyout â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {selectedItem && (
      ...
      {showBarcodePrint && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowBarcodePrint(false)}>
          <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-200"><h2 className="text-sm font-semibold text-gray-900">ðŸ· Print Barcode Labels</h2><p className="text-xs text-gray-400 mt-0.5">Generate labels for pieces â€” scan-to-confirm receiving</p></div>
            <div className="px-6 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Select Cargo</label><select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">{MOCK_INVENTORY.map(i => <option key={i.id} value={i.id}>{i.mawb} â€” {i.consignee} ({i.pieces} pcs)</option>)}</select></div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="border border-gray-300 bg-white rounded-lg p-4 text-center" style={{ fontFamily: 'monospace' }}>
                  ...
                  <div className="text-lg font-bold mt-1">â– â– â– â– â– â– â– â– â– â– â– â– </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div><label className="block text-xs text-gray-500 mb-1">Printer</label><select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option>Zebra ZT230 â€” Warehouse</option><option>Zebra GK420d â€” Receiving</option><option>PDF (Download)</option></select></div>
      ...
      {showPhotoCapture && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowPhotoCapture(false)}>
          <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-200"><h2 className="text-sm font-semibold text-gray-900">ðŸ“· Photo Capture â€” Receiving Inspection</h2><p className="text-xs text-gray-400 mt-0.5">Document cargo condition at receiving for damage claims</p></div>
            <div className="px-6 py-4 space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Select Cargo</label><select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">{MOCK_INVENTORY.map(i => <option key={i.id} value={i.id}>{i.mawb} â€” {i.consignee} ({i.pieces} pcs)</option>)}</select></div>
              <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Condition</label><select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option>Good â€” No visible damage</option><option>Minor â€” Cosmetic scuffs/dents</option><option>Moderate â€” Packaging compromised</option><option>Severe â€” Visible content damage</option><option>Wet / Water damage</option></select></div>
