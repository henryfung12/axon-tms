...
const MOCK_AUDIT: AuditEntry[] = [
  { id: 'au1', timestamp: '2026-04-14T14:22:00Z', user: 'Karen Liu', userRole: 'Operations', module: 'BROKERAGE', category: 'SHIPMENT', action: 'STATUS_CHANGED', entity: 'SH-10421', entityId: 's1', description: 'Shipment status changed to In Transit', details: 'Status: DISPATCHED Ã¢’ IN_TRANSIT. Carrier: Eagle Freight Lines. Driver confirmed pickup at Acme Detroit.', ipAddress: '10.0.1.42' },
  { id: 'au2', timestamp: '2026-04-14T14:18:00Z', user: 'System (Trucker Tools)', userRole: 'System', module: 'SYSTEM', category: 'TRACKING', action: 'UPDATED', entity: 'SH-10421', entityId: 's1', description: 'GPS location updated via Trucker Tools', details: 'Location: Bowling Green, KY 42101. Speed: 62 mph. Heading: South.', ipAddress: 'Ã¢' },
  ...
  { id: 'au5', timestamp: '2026-04-14T13:30:00Z', user: 'System (CargoWise)', userRole: 'System', module: 'CARGOWISE', category: 'INVOICE', action: 'SYNCED', entity: 'INV-20260401', entityId: 'cw1', description: 'Invoice synced to CargoWise', details: 'CW Job: CW-BRK-20260401. Status: INVOICE_MATCHED. Amount: $2,800.', ipAddress: 'Ã¢' },
  { id: 'au6', timestamp: '2026-04-14T13:15:00Z', user: 'System (EDI)', userRole: 'System', module: 'EDI', category: 'SHIPMENT', action: 'CREATED', entity: 'SH-10428', entityId: 's8', description: 'EDI 204 Load Tender received and auto-accepted', details: 'Partner: Acme Manufacturing (ISA: ACME001). Route: Detroit, MI Ã¢’ Columbus, OH. Weight: 45,000 lbs.', ipAddress: 'Ã¢' },
  { id: 'au7', timestamp: '2026-04-14T12:45:00Z', user: 'Priya Patel', userRole: 'Sales', module: 'BROKERAGE', category: 'RATE', action: 'UPDATED', entity: 'Rate: DETÃ¢’CHI', entityId: 'r1', description: 'Contract rate updated', details: 'Customer: Acme Manufacturing. Old rate: $2,600. New rate: $2,800. Effective: 2026-01-01.', ipAddress: '10.0.1.55' },
  { id: 'au8', timestamp: '2026-04-14T12:30:00Z', user: 'Marcus Johnson', userRole: 'Driver', module: 'DRIVER_APP', category: 'LOAD', action: 'STATUS_CHANGED', entity: 'LD-4521', entityId: 'L001', description: 'Driver updated load status: At Delivery', details: 'Status: IN_TRANSIT Ã¢’ AT_DELIVERY. Location: Nashville, TN. Via Driver Mobile App.', ipAddress: '72.84.102.55' },
  ...
  { id: 'au13', timestamp: '2026-04-14T10:30:00Z', user: 'System (RMIS)', userRole: 'System', module: 'SYSTEM', category: 'CARRIER', action: 'UPDATED', entity: 'Carrier: Eagle Freight Lines', entityId: 'cr1', description: 'RMIS monitoring alert Ã¢insurance updated', details: 'Auto Liability policy renewed. New expiry: 2027-02-15. Carrier: Progressive Commercial.', ipAddress: 'Ã¢' },
  { id: 'au14', timestamp: '2026-04-14T10:00:00Z', user: 'System (RTS Financial)', userRole: 'System', module: 'SYSTEM', category: 'PAYMENT', action: 'APPROVED', entity: 'Quick Pay: BL-EFL-4521', entityId: 'f1', description: 'Quick pay funded via RTS Financial', details: 'Amount: $2,200. Fee: $66 (3%). Net: $2,134. Funded in 2 hours.', ipAddress: 'Ã¢' },
  ...
  { id: 'au16', timestamp: '2026-04-14T09:15:00Z', user: 'Karen Liu', userRole: 'Operations', module: 'BROKERAGE', category: 'SHIPMENT', action: 'CREATED', entity: 'SH-10432', entityId: 's12', description: 'New shipment created', details: 'Customer: Heartland Foods. Route: Omaha, NE Ã¢’ Denver, CO. Rate: $3,100. Equipment: Reefer.', ipAddress: '10.0.1.42' },
  { id: 'au17', timestamp: '2026-04-14T09:00:00Z', user: 'Mike Santos', userRole: 'Manager', module: 'BROKERAGE', category: 'CUSTOMER', action: 'UPDATED', entity: 'Customer: Acme Manufacturing', entityId: 'c1', description: 'Customer credit limit updated', details: 'Credit limit: $75,000 Ã¢’ $100,000. Approved by Mike Santos.', ipAddress: '10.0.1.38' },
  ...
  { id: 'au19', timestamp: '2026-04-14T08:30:00Z', user: 'System (DOE)', userRole: 'System', module: 'SYSTEM', category: 'RATE', action: 'UPDATED', entity: 'DOE Diesel Index', entityId: '', description: 'DOE National Average Diesel price updated', details: 'New price: $3.824/gal. Change: -$0.012. FSC/mile: $0.30. Week ending 4/14/2026.', ipAddress: 'Ã¢' },
...
const ACTION_BADGE: Record<string, { bg: string; icon: string }> = {
  ...
  ASSIGNED: { bg: 'bg-purple-100 text-purple-800', icon: 'Ã¢’' },
  UPLOADED: { bg: 'bg-teal-100 text-teal-800', icon: 'Ã¢‘' },
  SYNCED: { bg: 'bg-indigo-100 text-indigo-800', icon: '' },
  SENT: { bg: 'bg-cyan-100 text-cyan-800', icon: 'Ã¢—' },
  APPROVED: { bg: 'bg-green-100 text-green-800', icon: 'Ã¢Å““' },
  REJECTED: { bg: 'bg-red-100 text-red-800', icon: 'Ã¢Å“•' },
  LOGIN: { bg: 'bg-gray-100 text-gray-700', icon: 'Ã°Å¸‘' },
  EXPORTED: { bg: 'bg-orange-100 text-orange-800', icon: '' },
};

const CATEGORY_ICON: Record<string, string> = {
  LOAD: 'Ã°Å¸“Â¦', SHIPMENT: '', INVOICE: '', CARRIER: 'Ã°Å¸Â¢', CUSTOMER: 'Ã°Å¸‘Â¥', DRIVER: 'Ã°Å¸‘Â¤',
  ASSET: '', SETTINGS: '', USER: 'Ã°Å¸Â', DOCUMENT: 'Ã°Å¸“', CLAIM: '', RATE: 'Ã°Å¸’°',
  PAYMENT: 'Ã°Å¸’Â³', TRACKING: 'Ã°Å¸“Â¡', AUTH: 'Ã°Å¸‘',
};
          ...
          <div className="relative w-[460px] bg-white shadow-2xl h-full overflow-y-auto border-l border-gray-200" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setSelectedEntry(null)} className="text-gray-400 hover:text-gray-600 text-lg">Ãƒ—</button>
              </div>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-3 space-y-1.5">
                ...
                <div className="flex justify-between text-xs"><span className="text-gray-400">Entity ID</span><span className="font-mono text-gray-600">{selectedEntry.entityId || 'Ã¢'}</span></div>
              </div>
            </div>
          </div>
        </div>
