    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={exportClaims} className="px-4 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">â¬‡ Export</button>
        ...
        <table className="w-full text-sm"><thead><tr className="bg-gray-50 border-b border-gray-200">
          ...
          {filtered.map(c => (
            <tr key={c.id} className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${c.status === 'DENIED' ? 'opacity-50' : c.claimAmount >= 10000 ? 'bg-red-50 border-l-4 border-l-red-400' : ''}`} onClick={() => setSelectedClaim(c)}>
              ...
              <td className="px-3 py-2.5 text-right font-medium text-green-600">{c.approvedAmount > 0 ? fmtCurrency(c.approvedAmount) : 'â€”'}</td>
              <td className="px-3 py-2.5 text-right font-medium">{c.paidAmount > 0 ? <span className="text-green-600">{fmtCurrency(c.paidAmount)}</span> : 'â€”'}</td>
      ...
      {/* â”€â”€ Claim Detail Flyout â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {selectedClaim && (
          ...
          <div className="relative w-[500px] bg-white shadow-2xl h-full overflow-y-auto border-l border-gray-200" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setSelectedClaim(null)} className="text-gray-400 hover:text-gray-600 text-lg">Ã—</button>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-green-50 rounded-lg p-2.5 text-center"><p className="text-lg font-bold text-green-600">{selectedClaim.approvedAmount > 0 ? fmtCurrency(selectedClaim.approvedAmount) : 'â€”'}</p><p className="text-xs text-gray-500">Approved</p></div>
                <div className="bg-blue-50 rounded-lg p-2.5 text-center"><p className="text-lg font-bold text-blue-600">{selectedClaim.paidAmount > 0 ? fmtCurrency(selectedClaim.paidAmount) : 'â€”'}</p><p className="text-xs text-gray-500">Paid</p></div>
              </div>
                ...
                <div className="space-y-1.5">
                  {selectedClaim.documents.map((d, i) => (
                    <div key={i} className="flex items-center justify-between py-1.5 px-3 bg-gray-50 rounded-lg">
                      <div><p className="text-xs font-medium text-blue-600">{d.name}</p><p className="text-xs text-gray-400">{d.type} Â· {fmtDate(d.date)}</p></div>
                      <button className="text-xs text-gray-400 hover:text-blue-600">â¬‡</button>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {selectedClaim.notes.map((n, i) => (
                    <div key={i} className="border-l-2 border-blue-200 pl-3">
                      <p className="text-xs text-gray-400">{fmtDT(n.date)} â€” <span className="font-medium text-gray-600">{n.user}</span></p>
      ...
      {/* â”€â”€ File New Claim Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {showAddModal && (
          ...
          <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-red-400 hover:bg-red-50">
                ...
                <p className="text-xs text-gray-400 mt-1">PDF, JPG, PNG, ZIP â€” up to 25 MB</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
